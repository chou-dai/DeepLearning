...
# 学習
history = model_gru.fit(x, t, epochs=20, batch_size=batch_size, validation_split=0.1)


# sin関数の次の値の予測
predicted = x[0].reshape(-1)  # 最初の入力。reshape(-1)で一次元のベクトルにする。
for i in range(0, n_sample):
    y = model_gru.predict(predicted[-n_rnn:].reshape(1, n_rnn, 1))  # 直近のデータを使って予測を行う（バッチサイズ,時系列数,入力層のニューロン数）
